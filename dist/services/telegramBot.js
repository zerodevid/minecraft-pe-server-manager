...
const bedrockProcess_1 = __importDefault(require("./bedrockProcess"));
const SERVER_PROPERTIES = path_1.default.join(config_1.BDS_DIR, 'server.properties');
class TelegramService {
    constructor() {
        this.bot = null;
        this.config = appSettings_1.default.getTelegram();
        this.lastPlayers = [];
        ...
        this.bot.on('message', async (msg) => {
            ...
            if (text.startsWith('/')) {
                ...
                switch (command) {
                    case '/status':
                        isBotCommand = true;
                        const status = bedrockProcess_1.default.getStatus();
                        const players = bedrockProcess_1.default.getPlayers();
                    ...
                    case '/users':
                        isBotCommand = true;
                        const activePlayers = bedrockProcess_1.default.getPlayers();
                    ...
                    case '/restart':
                        ...
                        const restartRes = await bedrockProcess_1.default.restartServer();
                    ...
                    case '/kick':
                        ...
                        if (!args) {
                            ...
                            const kickResult = bedrockProcess_1.default.sendCommand(kickCmd);
                            if (kickResult.sent) {
                    ...
                    case '/ban':
                        ...
                        if (!args) {
                            ...
                            const banResult = bedrockProcess_1.default.sendCommand(banCmd);
                            if (banResult.sent) {
            ...
            if (!isBotCommand) {
                ...
                if (!cmdToSend.trim())
                    return;
                const result = bedrockProcess_1.default.sendCommand(cmdToSend);
                if (result.sent) {
    ...
    getServerPort() {
        try {
            if (!fs_1.default.existsSync(SERVER_PROPERTIES)) {
                return null;
            }
            const content = fs_1.default.readFileSync(SERVER_PROPERTIES, 'utf-8');
            ...
            console.warn('Unable to read server.properties:', err.message);
        }
        return null;
    }
    getServerIpAddress() {
        ...
        for (const entries of Object.values(interfaces)) {
            ...
            for (const detail of entries) {
                if (!detail)
                    continue;
                const familyValue = detail.family;
                const isIPv4 = (typeof familyValue === 'string' && familyValue === 'IPv4') ||
    ...
    setupListeners() {
        ...
        bedrockProcess_1.default.on('players', (currentPlayers) => {
            // Check for joins
        ...
        bedrockProcess_1.default.on('output', (line) => {
    ...
    startHourlyStatus() {
        ...
        this.hourlyInterval = setInterval(() => {
            if (!this.config?.events?.hourlyStatus || !this.bot)
                return;
            const status = bedrockProcess_1.default.getStatus();
            const players = bedrockProcess_1.default.getPlayers();
